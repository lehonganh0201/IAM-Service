...
values
    ('USER_CREATE', 'Create user', 'Create user', false, now(), now()),
    ...
    ('PERMISSION_DELETE', 'Delete permission', 'Delete permission', false, now(), now())
    on conflict do nothing;

insert into roles(code, name, description, deleted, created_at, updated_at)
values
    ('USER_MANAGER', 'User Manager', 'Manage users', false, now(), now()),
    ('SYSTEM_ADMIN', 'System Admin', 'Manage roles and permissions', false, now(), now())
    on conflict do nothing;